...
Cookie autosync now seeds both `accounts.json` and the upstream `gemini_webapi` cookie cache from the configured browser profile, so startup can reuse the broader Google session state that a browser-backed Gemini login already established.
